...
import com.conboi.core.domain.MAX_ADD_TASK
...
constructor(
    ...
    fun increaseRateUs(count: Int = 1) =
        repeat(count) {
            viewModelScope.launch {
    ...
    fun onHideOvercompletedClick(hideOvercompleted: Boolean) =
        viewModelScope.launch {
            withContext(ioDispatcher) {
        ...
        if (getUser()?.isEmailVerified == true) {
            viewModelScope.launch {
                withContext(ioDispatcher) {
                    ...
                    val resultTasks =
                        firebaseRepository.uploadTasks(
                            list,
                            isBackupDownloaded,
    ...
    fun downloadLatestBackupInfo(callback: (Long?, Exception?) -> Unit) {
        viewModelScope.launch {
